...
function PositionListPage(props) {
  ...
  const fetchAllPositions = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/positions/positionlist`, {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
      setAllPositionData(response.data?.data)
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const columns = ['No', 'Position']

  const data = allPositionData.map((position, index) => ({
  ...
  return (
    <Layout>
      <Flex justifyContent="space-between" alignItems="center">
        <Text fontWeight="bold" fontSize="x-large">Positions</Text>
      ...
      <Box mt={10}>
        <Flex justifyContent="space-between" alignItems="center" mb={4}>
          ...
          />
          <Button colorScheme="green"> Add Position</Button>
        </Flex>
